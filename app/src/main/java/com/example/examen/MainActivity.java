...
import com.example.examen.ml.ModelDataSet;
        ...
        implements OnSuccessListener<Text>, OnFailureListener, ImageAnalysis.Analyzer, ImageReader.OnImageAvailableListener
{
    ...
    public void PersonalizedModel(View v) {
        try {
            ModelDataSet model = ModelDataSet.newInstance(getApplicationContext());
            TensorImage image = TensorImage.fromBitmap(mSelectedImage);

            ModelDataSet.Outputs outputs = model.process(image);
    ...
    private void processImage() {
        ...
        try {
            ...
            String res="";
            for (int i = 0; i < probability.size(); i++)
                res = es + probability.get(i).getLabel() +  " " +  probability.get(i).getScore()*100 + " % \n";

            String url ="http://www.geognos.com/api/en/countries/info/"+es+".json";
            StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                    new Response.Listener<String>() {
                        @Override
                        public void onResponse(String response) {
                            // Display the first 500 characters of the response string.
                            Descripcion=response;//.substring(0,5000)
                        }
                    }, new Response.ErrorListener() {
                @Override
                public void onErrorResponse(VolleyError error) {
                    Descripcion="No se reconocio la bandera!";
                }
            });
